...
from utils import get_residual_features, get_matched_ref_features,get_matched_ref_features_top
...
def validate1(args, encoder, vq_ops, constraintor, estimators, test_loader, ref_features, device, class_name):
    vq_ops.eval()
    constraintor.eval()
    ...
    for idx, batch in enumerate(test_loader):
        ...
        with torch.no_grad():
            if args.backbone == 'wide_resnet50_2':
                ...
                features = encoder(image)
                mfeatures = get_matched_ref_features_top(features, ref_features,args.rank)
                rfeatures = get_residual_features(features, mfeatures, pos_flag=True)
            else:
