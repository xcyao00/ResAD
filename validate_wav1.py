...
# 引数に gating_net を追加
def validate(args, encoder, constraintor, gating_net, wav_filter, estimators, test_loader, ref_features, device, class_name):
    gating_net.eval() # ゲーティングネットワークも評価モードに
    constraintor.eval()
    ...
    for idx, batch in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # 2. 画像の深い特徴量から層ごとの重みを予測 [w1, w2, w3]
            w1, w2, w3 = gating_net(features_raw[-1].detach())
            weights = [w1, w2, w3]
            
            # 3. 生のカンペと生の特徴量をマッチング
            mfeatures_raw = get_matched_ref_features(features_raw, ref_features)
            
            # 4. テスト画像とカンペの両方に、層ごとの重みでウェーブレットを適用
